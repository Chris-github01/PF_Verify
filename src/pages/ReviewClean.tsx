...
import { Trash2, Edit2, Check, X, Wand2, AlertCircle, Target, Sparkles, Zap, Play } from 'lucide-react';
...
export default function ReviewClean({ projectId, onNavigateBack, onNavigateNext }: ReviewCleanProps) {
  ...
  const handleCleanAndMapSelectedQuote = async () => {
    ...
    if (normalising || mapping || smartCleaning) return;
  ...
  const handleProcessAllPendingQuotes = async () => {
    ...
    if (pendingQuotes.length === 0) {
      setMessage({ type: 'info', text: 'No pending quotes to process.' });
      return;
    }
    ...
    try {
      for (let i = 0; i < pendingQuotes.length; i++) {
        ...
        if (!quoteItems || quoteItems.length === 0) {
          ...
          await supabase
            .from('quotes')
            .update({ status: 'error', processing_error: 'No items found' })
            .eq('id', quote.id);
          continue;
        }
